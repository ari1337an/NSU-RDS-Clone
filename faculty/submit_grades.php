...
//     global $APP_DB;
//     $res = $APP_DB->query("
//             SELECT round(sum(grade_value),0) as 'total_grade', count(*) as 'total_course'
//             FROM grades
//             GROUP by given_to
//             HAVING given_to=" . USERS::getUserName());
//     // var_dump($res);
//     if (mysqli_num_rows($res) > 0) {
//         $row = mysqli_fetch_assoc($res);
//         $total_grade = $row['total_grade'];
//         $total_course = $row['total_course'];
//         $cgpa = $total_grade / $total_course;
...
?>
    <select name="grade_for_<?php echo $id; ?>">
        ...
        ?>
            <option value="<?php echo $grade_single; ?>" <?php if ($flag_exists && $val == $grade_single) {
                                                            echo "selected";
                                                        } ?>><?php echo $grade_single; ?></option>
        <?php
        }
        ?>
        <option value="None" <?php if (!$flag_exists) {
                                    echo "selected";
                                } ?>>None</option>
    </select>
<?php
...
<body>
    ...
    <h2 class="table_desc">Submitting Grades For <?php echo $_GET['course_id']; ?></h2>
...
");
            ...
            <br>
            <input class="btn btn-green" style="position: absolute;right: 130px;" type="submit" name="request_for_grade_change" value="Submit">
        </form>
    </section>
